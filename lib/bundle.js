...
/***/ function(module, exports, __webpack_require__) {
	...
	class Simba extends MovingObject {
	  ...
	  getBob() {
	    ...
	    for (let i = 1; i <= 8; i++) {
	      let img = new Image();
	      img.src = `assets/sprites/bob/bob${i}.png`;
	      this.BobImages.push(img);
	    }
	  }

	  drawBob() {
	    if (this.state.grounded === false) {
	      return this.BobImages[6];
	    } else {
	      return this.BobImages[this.currentFrame];
	    }
	  }
	}
...
/***/ function(module, exports) {
	...
	class Display {
	  ...
	  drawTopPlayer(ctx) {
	    ...
	    ctx.font = "14px Lato";
	    ctx.fillStyle = "yellow";
	    ctx.fillText(" Top Player:  "+topPlayer, 12, 47);
	  }

	  drawDate(ctx) {
	    let date = 'Loading..';
	    firebase.database().ref('/date').on('value', snapshot => {
	      date = snapshot.val().slice(0, 10);
	    });

	    ctx.font = "14px Lato";
	    ctx.fillStyle = "yellow";
	    ctx.fillText("  Scored on:  "+date, 12, 32);
	  }
